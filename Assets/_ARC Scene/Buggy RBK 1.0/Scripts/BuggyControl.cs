...
public class BuggyControl : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        wheels[0] = SetWheelComponent(carWheels.wheels.frontRight, carWheels.wheels.frontRightAxle,
            carSetting.maxSteerAngle, carWheels.wheels.frontWheelDrive, carWheels.wheels.frontRight.position.y);
        wheels[1] = SetWheelComponent(carWheels.wheels.frontLeft, carWheels.wheels.frontLeftAxle,
            carSetting.maxSteerAngle, carWheels.wheels.frontWheelDrive, carWheels.wheels.frontLeft.position.y);

        wheels[2] = SetWheelComponent(carWheels.wheels.backRight, carWheels.wheels.backRightAxle, 0,
            carWheels.wheels.backWheelDrive, carWheels.wheels.backRight.position.y);
        wheels[3] = SetWheelComponent(carWheels.wheels.backLeft, carWheels.wheels.backLeftAxle, 0,
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        {
            collision.transform.root.GetComponent<BuggyControl>().slip2 =
                Mathf.Clamp(collision.relativeVelocity.magnitude, 0.0f, 5.0f);
    ...
    void FixedUpdate()
    {
        ...
        if (activeControl)
        {
            if (controlMode == ControlMode.simple)
            {
                ...
                {
                    steer = Mathf.MoveTowards(steer, inputs.Horizontal, 0.2f);
                    accel = inputs.Vertical;
                    brake = inputs.Brake;
                    shift = inputs.Shift;
                }
            }
            ...
            {
                accel = (accelFwd == 0 ? accelBack : accelFwd);
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            if (col.GetGroundHit(out hit))
            {
                if (carParticles.brakeParticlePerfab)
                {
                    ...
                    {
                        Particle[currentWheel] = Instantiate(carParticles.brakeParticlePerfab,
                            w.wheel.position - (Vector3.up * (w.collider.radius - 0.5f)),
